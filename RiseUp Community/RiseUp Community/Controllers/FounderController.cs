...
namespace RiseUp_Community.Controllers
{
    public class FounderController : Controller
    {
        ...
        // 2. Pitch Startup Button
        [HttpGet]
        public IActionResult PitchStartup()
        {
            return View(); // Views/Founder/PitchStartup.cshtml এ রিডাইরেক্ট করবে
        }
        ...
        {
            return View(); // Views/Founder/PostIdea.cshtml
        }

        // 4. Search Investors Tab / Button
        public IActionResult SearchInvestors()
        {
            return View(); // Views/Founder/SearchInvestors.cshtml
        }

        // 5. Find Mentors Tab / Button
        public IActionResult FindMentors()
        {
            return View(); // Views/Founder/FindMentors.cshtml
        }

        // 6. Edit Pitch Button
        [HttpGet]
        public IActionResult EditPitch(int id)
        {
            return View(); // Views/Founder/EditPitch.cshtml
        }

        // 7. Upload Pitch Deck PDF
        [HttpPost]
        public async Task<IActionResult> UploadPitchDeck(IFormFile pitchDeckPdf)
        {
            if (pitchDeckPdf != null && pitchDeckPdf.Length > 0)
            {
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/uploads", pitchDeckPdf.FileName);
                using (var stream = new FileStream(filePath, FileMode.Create))
                {
                    await pitchDeckPdf.CopyToAsync(stream);
                }
                TempData["Success"] = "Pitch Deck Uploaded Successfully!";
            }
            return RedirectToAction("Dashboard");
        }

        // 8. Accept & Open Chat Button (Investor Interest)
        public IActionResult OpenChat(string investorId)
        {
            return RedirectToAction("Index", "Chat", new { receiverId = investorId });
        }

        // 9. Send Pitch Deck Button
        [HttpPost]
        public IActionResult SendPitchDeck(string investorId)
        {
            TempData["Success"] = "Pitch Deck sent to investor!";
            return RedirectToAction("Dashboard");
        }
        ...
        public override ObjectResult Problem(string? detail, string? instance, int? statusCode, string? title, string? type)
        {
            return base.Problem(detail, instance, statusCode, title, type);
        }

        public override ObjectResult Problem(string? detail = null, string? instance = null, int? statusCode = null, string? title = null, string? type = null, IDictionary<string, object?>? extensions = null)
        {
            return base.Problem(detail, instance, statusCode, title, type, extensions);
        }
        ...
        public override ActionResult ValidationProblem(string? detail, string? instance, int? statusCode, string? title, string? type, [ActionResultObjectValue] ModelStateDictionary? modelStateDictionary)
        {
            return base.ValidationProblem(detail, instance, statusCode, title, type, modelStateDictionary);
        }

        public override ActionResult ValidationProblem(string? detail = null, string? instance = null, int? statusCode = null, string? title = null, string? type = null, [ActionResultObjectValue] ModelStateDictionary? modelStateDictionary = null, IDictionary<string, object?>? extensions = null)
        {
            return base.ValidationProblem(detail, instance, statusCode, title, type, modelStateDictionary, extensions);
        }
